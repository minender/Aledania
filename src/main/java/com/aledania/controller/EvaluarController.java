...
import com.aledania.lambdacalculo.Corrida;
import com.aledania.lambdacalculo.Term;
import com.aledania.lambdacalculo.Term.Redex;
import com.aledania.lambdacalculo.Tripla;
...
public class EvaluarController {
    ...
    public String evaluarPasoAPasoView(@Valid InsertarEvaluar insertarEvaluar, BindingResult bindingResult, @PathVariable String username, ModelMap map)
    {
            ...
            if( bindingResult.hasErrors() )
            {
                ...
                map.addAttribute("fromEvalError","../perfil/"+username+"/");
		...
		String msg = parser.getErrorMessage(e, TermParser.tokenNames);
            ...
            List<String> alias=tripla.alias;
            List<String> valores=tripla.valores;
            String steps = insertarEvaluar.getNombre();
            Corrida corr;
            if (steps.equals(""))
               corr=term.evaluarFinal(500);
            else
               corr=term.evaluarFinal(Integer.parseInt(steps));
           ...
           //corr.terminos.set(0, tripla.term.replace("\\", "\\\\"));
            ...
            map.addAttribute("term",tripla.term);
            if(alias.size()!= 0)
                map.addAttribute("nAlias",new Integer(alias.size()-1));
            else
            {
